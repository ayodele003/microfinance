...
$dir = "../../skyeSave/upload/SaveFiles/"; // Upload directory
	...
	$uext = pathinfo($_FILES['ufile']['name'], PATHINFO_EXTENSION);
...
$uext = strtolower($uext);
	...
	if ( in_array($uext, $valid_formats)) {
		if( $_FILES['ufile']['size'] < $max_file_size ){
			$uuniq = base_convert(uniqid(), 16, 10);
			$utmp = $_FILES['ufile']['tmp_name'];
			$uuniq_file_name = $uuniq.".".$uext;
			if(move_uploaded_file($utmp, $dir.$uuniq_file_name)){
				}
			else{
				$umsg = "Please upload your Utility Bill";
			}
		}
		else{
			$umsg = "Utility Bill File is too large";
		}
	}
	else{
		$umsg = "Utility Bill has wrong file format";
...
$AcctType = "New Polaris Save Plus Savings";
...
$RefCode = 'SB-SAVE-' .date("d") . date("m") . date("Y") .'-'. $zrefno;	
...
$fx=mysql_query("select Fname, Lname, DateOfBirth, CellPhone from save_accounts WHERE Fname = '$fname' AND Lname='$lname' AND DateOfBirth = '$DateOfBirth' AND CellPhone='$CellPhone'") or die(mysql_error());
$numrows = mysql_num_rows($fx);
if($numrows > 0){
echo "Dear $fname, <br><br> Sorry, we noticed that an existing Polaris Save Plus  application match your details. Duplicate applications are not allowed."; 
exit;
}

$fxx=mysql_query("select CellPhone from save_accounts WHERE CellPhone='$CellPhone'") or die(mysql_error());
$numrows = mysql_num_rows($fxx);
if($numrows > 0){
echo "Dear $fname, <br><br> Sorry, we noticed that an existing Polaris Save Plus application match your phone number. Duplicate phone numbers are not allowed."; 
exit;
}


$sqli = "INSERT INTO save_accounts (RefCode, AcctType, Title, Fname, Mname, Lname, FullName, DateOfBirth, PlaceOfBirth, StateofOrigin, Religion, LGA, Sex, MaritalStatus, ResidentialAddress, NBS, RPostal, RCity, RState, RLga, RCountry, MailingAddress, Telephone, CellPhone, Email, MeansofID, IDNumber, IDIssueDate, IDExpiryDate, EmployStatus, Occupation, Income, `Usage`, EmployerName, EmployerAddress, NextOfKin, Relationship, AddressOfNOK, EmailOfNOK, PhoneOfNOK, Services, Security, PreferredBranch, Referral, Passport, Signature, Identification,UBill,PVR, bvn, Status, StatusDate, Comments, Comments2, CSO, BSM, DateApproved, DateDeclined, DateTreated, DateFiled, AcctNum) 
VALUES ('$RefCode', '$AcctType', '$title', '$fname', '$mname', '$lname', '$FullName', '$DateOfBirth', '$PlaceOfBirth', '$StateofOrigin', '$Religion', '$LGA', '$Sex', '$MaritalStatus', '$ResidentialAddress', '$nbs', '$rpostal', '$rcity', '$rstate', '$rlga', '$rcountry', '$MailingAddress', '$TelePhone', '$CellPhone', '$Email', '$meansofid', '$idno', '$issdate', '$exdate', '$EmployStatus', '$Occupation', '$income', '$use', '$EmployerName', '$EmployerAddress', '$NextOfKin', '$Relationship', '$AddressOfNOK', '', 0, '".$Services[0].", ".$Services[1].", ".$Services[2]."', '$Security', '$PreferredBranch', '$Referral', '{$puniq_file_name}', '{$suniq_file_name}', '{$iduniq_file_name}','{$uuniq_file_name}', '','$bvn', '', CURRENT_TIMESTAMP, '', '', '', '', '', '', '', '', '')";		   	   
...
$messagex = "<table width=550 border=0 cellspacing=0 cellpadding=5> <tr>
<td bgcolor=#efefef><table width=600 border=0 cellspacing=0 cellpadding=5>
<tr><td bgcolor=#FFFFFF style=border-top:2px solid #efefef;font-family:Verdana, Arial, Helvetica, sans-serif; font-size:14px><p align=center><strong>Polaris Save Plus Application</strong></p><blockquote>Dear $FullName, <br><br>Your application for $AcctType has been successfully logged into our system.<br><br>We are in the process of creating your new account. Please quote the Reference Code below in any future correspondence regarding this application.<br><br><b>Reference Code: $RefCode</b><br><br><br><br>Please, do not hesitate to contact us via our email: yescenter@polarisbanklimited.com and phone numbers:01-4482100 or 08069880000, for any questions or enquiries you may have.<br><br><br>Best Regards. <br><br><br>Polaris Save Plus Team</td></tr></table></td></tr></table>";
